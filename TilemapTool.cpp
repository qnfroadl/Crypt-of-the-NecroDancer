    ...
    if (PtInRect(&rcSampleTile, g_ptMouse)) {
        ...
        if (lStay || rStay) {
            ...
            case ToolType::WALL_TILE: {
                ...
                Block* oldBlock = tiles[x][y]->GetBlock();
                if (oldBlock) delete oldBlock;

                Block* block = new Block();
                block->Init(tiles[x][y]->GetPos(), tiles[x][y]->GetTileIndex());
...
void TilemapTool::Save(string filePath)
{
    ...
    for (int y = 0; y < mapSize; y++) {
        for (int x = 0; x < mapSize; x++) {
            Block* block = tiles[x][y]->GetBlock();
            out << (block ? block->GetBlockNum() : -1) << " ";
        }
        out << endl;
    }
...
void TilemapTool::Load(string filePath)
{
    ...
    string section;
    in >> section;
    if (section == "PLAYER_START") in >> playerStartX >> playerStartY;
    in >> section;
    if (section == "NEXT_STAGE") in >> nextStageX >> nextStageY;
    in >> section;
